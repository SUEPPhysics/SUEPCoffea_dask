...
def plot(ievt, tracks, genParticles, boost=False, ax=None, showGen=False, 
         showPFCands=True, showRingOfFire=True, params=None, showCandidates=False):
    ...
    if showPFCands: # plot PFCands
        ax.scatter(tracks.phi, tracks.eta,
                ...
                c='xkcd:gray', marker='o')
        
    if not boost:

        # Add jet info to the plot
        ax.scatter(jetsAK15.phi, jetsAK15.eta, s=scale(jetsAK15,scalarParticle),
                facecolors='none', edgecolors='xkcd:green')
        for jet in jetsAK15:
            phis, etas = get_dr_ring(1.5, jet.phi, jet.eta)
            phis = phis[1:]
            etas = etas[1:]
            ax.plot(phis[phis>pi]-2*pi, etas[phis>pi], color='xkcd:green', linestyle='--')
            ax.plot(phis[phis<-pi]+2*pi, etas[phis<-pi], color='xkcd:green', linestyle='--')
            ax.plot(phis[phis<pi], etas[phis<pi], color='xkcd:green', linestyle='--')
    ...
    if showCandidates:
        ...
        _, _, topTwoJets = getTopTwoJets(
            None, ak.ones_like(jetsAK15), ak.ones_like(jetsAK15), # some moot arrays because this function needs rewriting
        ...
        if boost:
            ax.scatter(SUEP_cluster_tracks.phi, SUEP_cluster_tracks.eta,
                s=scale(tracks,scalarParticle),
                c='xkcd:red', marker='o')
            ax.scatter(ISR_cluster_tracks.phi, ISR_cluster_tracks.eta,
                s=scale(tracks,scalarParticle),
                c='xkcd:blue', marker='o')
            
        else:
            # Add SUEP candidate
            ax.scatter(SUEP_cand.phi, SUEP_cand.eta, s=scale(SUEP_cand,scalarParticle),
                    color='xkcd:red', marker='x')
            phis, etas = get_dr_ring(1.5, SUEP_cand.phi, SUEP_cand.eta)
            phis = phis[1:]
            etas = etas[1:]
            ax.plot(phis[phis>pi]-2*pi, etas[phis>pi], color='xkcd:red', linestyle='--')
            ax.plot(phis[phis<-pi]+2*pi, etas[phis<-pi], color='xkcd:red', linestyle='--')
            ax.plot(phis[phis<pi], etas[phis<pi], color='xkcd:red', linestyle='--')

            # Add ISR candidate
            ax.scatter(ISR_cand.phi, ISR_cand.eta, s=scale(ISR_cand,scalarParticle),
                    color='xkcd:blue', marker='x')
            phis, etas = get_dr_ring(1.5, ISR_cand.phi, ISR_cand.eta)
            phis = phis[1:]
            etas = etas[1:]
            ax.plot(phis[phis>pi]-2*pi, etas[phis>pi], color='xkcd:blue', linestyle='--')
            ax.plot(phis[phis<-pi]+2*pi, etas[phis<-pi], color='xkcd:blue', linestyle='--')
    ...
    if showRingOfFire and boost:
        ...
        ax.hlines(scalarParticle.eta + 1.0, xmin=-pi, xmax=pi, color='xkcd:red', alpha=0.7, linestyle='dotted') 
        ax.hlines(scalarParticle.eta - 1.0, xmin=-pi, xmax=pi, color='xkcd:red', alpha=0.7, linestyle='dotted') 
    ...
    line7 = ax.plot([-100], [-100], label='"Ring of fire"', linestyle='dotted',
                       c='xkcd:red')[0]
    line8 = ax.scatter([-100], [-100], label='Other AK15 jets' if showCandidates else 'AK15 Jets', marker='o',
                       facecolors='none', edgecolors='xkcd:green')
    line9 = ax.scatter([-100], [-100], label='AK15 SUEP Candidate', marker='x',
                       color='xkcd:red')
    line10 = ax.scatter([-100], [-100], label='AK15 ISR Candidate', marker='x',
                       color='xkcd:blue')
    ...
    ax.text(left+0.02, bottom+0.01, 'Scalar Mediator Frame' if boost else 'Lab Frame',
...
def main():
    ...
    parser.add_argument('-m', '--max', type=int, default=10, help='Max number of events to process')
    parser.add_argument('-b', '--boost', action='store_true', help='Show boosted to scalar mediator frame alongside lab frame')
    parser.add_argument('-bo', '--boostOnly', action='store_true', help='Show only boosted to scalar mediator (no lab frame)')
    parser.add_argument('-g', '--gen', action='store_true', help='Plot gen particles')
    parser.add_argument('-r', '--ring', action='store_true', help='Show ring of fire (only in boosted frame)')
    parser.add_argument('-p', '--pfcands', default=1, type=int, help='Show PFCands')
    parser.add_argument('-c', '--candidates', action='store_true', help='Show SUEP and ISR candidates (as per offline analysis definition)')
    ...
    for i in range(0, args.max):
        ...
        if args.boost:
            ...
            plot(i, this_tracks, this_genParticles, boost=False, ax=ax1, showCandidates=args.candidates,
                 params=params, showRingOfFire=args.ring, showGen=args.gen, showPFCands=args.pfcands)
            plot(i, this_tracks, this_genParticles, boost=True, ax=ax2, showCandidates=args.candidates,
                params=params, showRingOfFire=args.ring, showGen=args.gen, showPFCands=args.pfcands)
        elif args.boostOnly:
            fig = plt.figure(figsize=(6,7))
            ax1 = fig.subplots(1)
            hep.cms.label(llabel='Preliminary', data=False, ax=ax1)
            plot(i, this_tracks, this_genParticles, boost=True, showCandidates=args.candidates,
                 ax=ax1, params=params, showRingOfFire=args.ring, showGen=args.gen, showPFCands=args.pfcands)
        else:
            fig = plt.figure(figsize=(6,7))
            ax1 = fig.subplots(1)
            hep.cms.label(llabel='Preliminary', data=False, ax=ax1)
            plot(i, this_tracks, this_genParticles, boost=False, showCandidates=args.candidates,
                 ax=ax1, params=params, showRingOfFire=args.ring, showGen=args.gen, showPFCands=args.pfcands)
            
        # signal case
        if params:
            fig.savefig(args.output+'/mS-%d_mPhi-%d_T-%d_decay-%s_Event%d.pdf'%(*params,i))
            fig.savefig(args.output+'/mS-%d_mPhi-%d_T-%d_decay-%s_Event%d.png'%(*params,i))
        else:

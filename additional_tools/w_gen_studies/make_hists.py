...
def read_xsection(sample_name):
    with open('../../data/xsections_2018.json', 'r') as f:
...
def get_vars(events):
    ...
    pt = pt[Ws]
    pt = ak.fill_none(
        ak.pad_none(pt, 0 + 1, axis=1, clip=True), -999
    )[:, 0]
    eta = events['GenPart_eta'].array()
    eta = eta[Ws]
    eta = ak.fill_none(
        ak.pad_none(eta, 0 + 1, axis=1, clip=True), -999
    )[:, 0]
...
def main():
    ...
    for sample in samples:
        ...
        hists["HT_"+sampleshort] = Hist.new.Reg(20000, 0, 20000, name=f"HT", label="HT").Weight()
        hists["PT_"+sampleshort] = Hist.new.Reg(5000, 0, 5000, name=f"PT", label="PT").Weight()
        hists["ETA_"+sampleshort] = Hist.new.Reg(10000, -5, 5, name=f"ETA", label="ETA").Weight()
    ...
    for sample_name, sample_dir in samples.items():
        ...
        files = subprocess.check_output(command, shell=True, stderr=subprocess.PIPE).decode().splitlines()
        ...
        for file_index in range(min(N_FILES, len(files))):  
            ...
            totweight, genWeight, gen_W_pt, ht, eta = get_vars(tree)
        ...
        hists["ETA_"+sampleshort].fill(hts, weight=weight)

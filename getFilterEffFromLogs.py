    ...
    for filename in os.listdir(directory):
        ...
        if filename.endswith('.out'):
            ...
            if os.path.isfile(err_file_path):
                ...
                sample_name, nom, denom = extract_sample_info(out_file_path, err_file_path)
...
if __name__ == "__main__":
    ...
    for sample_name in xsections.keys():
        ...
        ms = float(ms.group(1))
        if ms in truth_values: continue
        truth_values[ms] = xsections[sample_name]['kr']


    for ms_value, (nom, denom, efficiency, efficiency_error) in averaged_efficiency_by_ms.items():

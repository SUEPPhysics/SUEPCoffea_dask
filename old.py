...
echo "xrdcp $2 input.root"
xrdcp $2 input.root

echo "python3 {condor_file} --jobNum=$1 --isMC={ismc} --era={era} --doInf={doInf} --doSyst={doSyst} --dataset={dataset} --infile=input.root"
python3 {condor_file} --jobNum=$1 --isMC={ismc} --era={era} --doInf={doInf} --doSyst={doSyst} --dataset={dataset} --infile=input.root
...
#+DESIRED_Sites         = "mit_tier2, T2_US_MIT"
        ...
        for sample in stream.read().split("\n"):
            ...
            with open(os.path.join(jobs_dir, "inputfiles.dat"), "w") as infiles:
                for i in Raw_list:
                    ...
                    # debug
                    i = i.replace("xrootd.cmsaf.mit.edu", "submit50.mit.edu")

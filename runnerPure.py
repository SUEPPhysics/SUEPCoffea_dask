...
def main():
        ...
        # -- out_vars_keys: list of all they keys used for output columns
        # -- output: dict to be passed to the accumulator
        # -- out_vars: pandas dataframe used to store & format the values temporarily
        #####################################################################################
        
        f0 = nanoevents.NanoEventsFactory.from_root(
            'root://cmseos.fnal.gov//store/user/lpcsuep/QCD_skimmed/merged/skimmed_300to470.root',
            # 'test.root',
            # "root://xcache//store/user/lpcsuep/QCD_skimmed/merged/skimmed_3200toInf.root",
        ...
        columns_IRM += ["SUEP_dphi_SUEP_ISR_IRM"]
        columns = columns_IRM + columns_CL + columns_CO + columns_CL_ISR + columns_CO_ISR
        ...
        output['out_weight'] = 0#processor.value_accumulator(float, 0)
        #if self.isMC:
        ...
        trigger_TripleMu = events.HLT.TripleMu_5_3_3_Mass3p8_DZ
        ...
        # from https://twiki.cern.ch/twiki/bin/view/CMS/JetID:
        ak4jets = ak4jets[clusterCut]
        events = events[clusterCut]
        # jetId==2 means: pass tight ID, fail tightLepVeto
        # jetId==6 means: pass tight and tightLepVeto ID.
        # need to pass clusterCut to these as well otherwise crashes
        ...
        if len(tracks) == 0:
            print(f"[{dataset}]: No events pass clusterCut.")
            for c in columns:
                out_vars[c] = np.nan
                output[c] = out_vars[c].to_list()
            return
        ...
        out_vars["HLT_TripleMu_5_3_3_Mass3p8_DZ"] = events.HLT.TripleMu_5_3_3_Mass3p8_DZ
        ...
        if not any(oneIRMtrackCut):
            print(f"[{dataset}]: No events in ISR Removal Method, oneIRMtrackCut.")
            ...
            # SUEP jet variables
            #eigs = math_utils.sphericity(
            #    self, SUEP_tracks_b_IRM, 1.0
            ...
            out_vars.loc[indices_IRM, "SUEP_rho0_IRM"] = math_utils.rho(
                None, 0, SUEP_cand_IRM, SUEP_tracks_IRM, deltaR
            )
            out_vars.loc[indices_IRM, "SUEP_rho1_IRM"] = math_utils.rho(
                None, 1, SUEP_cand_IRM, SUEP_tracks_IRM, deltaR
            )
        ...
        out_vars.loc[indices_CL, "SUEP_rho0_CL"] = math_utils.rho(
            None, 0, SUEP_cand_CL, SUEP_tracks_CL, deltaR
        )
        out_vars.loc[indices_CL, "SUEP_rho1_CL"] = math_utils.rho(
            None, 1, SUEP_cand_CL, SUEP_tracks_CL, deltaR
        )
        ...
        if not any(oneISRtrackCut):
            print(f"[{dataset}]: No events in Inverted CL Removal Method, oneISRtrackCut.")
            ...
            out_vars.loc[indices_CL, "ISR_rho0_CL"] = math_utils.rho(
                None, 0, ISR_cand_CL, ISR_tracks_CL, deltaR
            )
            out_vars.loc[indices_CL, "ISR_rho1_CL"] = math_utils.rho(
                None, 1, ISR_cand_CL, ISR_tracks_CL, deltaR
            )
        ...
        if not any(oneCOtrackCut):
            print(f"[{dataset}]: No events in Cone Method, oneCOtrackCut.")
            for c in columns_CO:
                out_vars[c] = np.nan
            if self.isMC:
            ...
            out_vars.loc[indices_CO, "SUEP_rho0_CO"] = math_utils.rho(
                None, 0, SUEP_cand_CO, SUEP_tracks_CO, deltaR
            )
            out_vars.loc[indices_CO, "SUEP_rho1_CO"] = math_utils.rho(
                None, 1, SUEP_cand_CO, SUEP_tracks_CO, deltaR
            )

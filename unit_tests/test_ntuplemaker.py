...
def test_ntuplemaker(run, config):
    ...
    for option, root_file in zip(options, root_files):
        command = f"python {script} --infile {root_file} {option} > {run}.out 2> {run}.err"
        ...
        if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
            print(colored(f"PASSED! Output file for run {run} exists and is not empty.", "green"))
        else:
            print(colored(f"FAILED! Output file for run {run} does not exist or is empty.", "red"))
            sys.exit()
    ...
    print(colored(f"The test for run {run} was a SUCCESS. Time: {end - start} seconds", "green"))

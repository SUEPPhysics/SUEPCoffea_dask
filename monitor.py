...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Move files to move_dir (/work/submit/) while you check if they are corrupted.",
    )
    ...
    out_dir = (
        "/mnt/T3_US_MIT/hadoop/scratch/" + username + "/SUEP/" + options.tag + "/{}/"
    )
                ...
                for file in filesToMove:
                    subprocess.run(
                        [
                            ...
                            + out_dir.split("hadoop")[-1].format(sample_name)
                            + "/"
                            + file,
                            move_dir.format(sample_name) + "/",
                        ]

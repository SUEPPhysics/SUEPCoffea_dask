...
def doBTagWeights(events, jetsPre, era, wp="L", do_syst=False):
    ...
    for syst_var in SF.keys(): # Method (1.a) here: https://twiki.cern.ch/twiki/bin/view/CMS/BTagSFMethods
       mceff   = ak.prod(np.where(jetsPre.btag >= btagcuts(wps[wp], era), effs, 1-effs), axis = 1)
       dataeff = ak.prod(np.where(jetsPre.btag >= btagcuts(wps[wp], era), SF[syst_var]*effs, 1-SF[syst_var]*effs), axis = 1)

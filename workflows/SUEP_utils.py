...
def ClusterMethod(
    ...
    # debug
    self.accumulator['vars'].loc(indices, "SUEP_nconst_CL" + out_label, ak.num(SUEP_tracks_b))        
    self.accumulator['vars'].loc(indices, "SUEP_nconst_CL" + out_label, ak.num(SUEP_tracks_b))
    self.accumulator['vars'].loc(indices, "SUEP_pt_avg_b_CL" + out_label, ak.mean(
        SUEP_tracks_b.pt, axis=-1
    ))
    self.accumulator['vars'].loc(indices, "SUEP_S1_CL" + out_label, 1.5 * (
        eigs[:, 1] + eigs[:, 0]
    ))

    # unboost for these
    SUEP_tracks = SUEP_tracks_b.boost_p4(SUEP_cand)
    self.accumulator['vars'].loc(indices, "SUEP_pt_avg_CL" + out_label, ak.mean(
    ...
    self.accumulator['vars'].loc(indices, "SUEP_delta_mass_genMass_CL" + out_label, (
        SUEP_cand.mass - self.accumulator['vars']["SUEP_genMass" + out_label][indices]
    ))
    self.accumulator['vars'].loc(indices, "SUEP_delta_pt_genPt_CL" + out_label, (
    ...
    if do_inverted:
        ...
        self.accumulator['vars'].loc(indices, "ISR_nconst_CL" + out_label, ak.num(ISR_tracks_b))
        self.accumulator['vars'].loc(indices, "ISR_pt_avg_b_CL" + out_label, ak.mean(
            ISR_tracks_b.pt, axis=-1
        ))
        self.accumulator['vars'].loc(indices, "ISR_S1_CL" + out_label, 1.5 * (
            eigs[:, 1] + eigs[:, 0]
        ))

        # unboost for these
        ISR_tracks = ISR_tracks_b.boost_p4(ISR_cand)
        self.accumulator['vars'].loc(indices, "ISR_pt_avg_CL" + out_label, ak.mean(

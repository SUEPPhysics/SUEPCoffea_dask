...
from workflows.CMS_corrections.btag_utils import btagcuts, getBTagEffs, doBTagWeights
import copy
import json
...
class SUEP_cluster_ZH(processor.ProcessorABC):
    ...
    def selectByLeptons(self, events, extraColls = []):
        ...
        cutElectrons = (events.Electron.pt >= 15) & (events.Electron.mvaFall17V2Iso_WP90) & ( abs(events.Electron.dxy) < (0.05 + 0.05*(abs(events.Electron.eta) > 1.479))) & (abs(events.Electron.dz) < (0.10 + 0.10*(events.Electron.eta > 1.479))) & ((abs(events.Electron.eta) < 1.444) | (abs(events.Electron.eta) > 1.566)) & (abs(events.Electron.eta) < 2.5)
    ...
    def process(self, events):
        ...
        if debug: print("Will check file %s"%fullFile)
        ...
        if not(self.isMC): self.doGen = False
	...
	# Sorting jets by pt.
        ...
        if self.doGen:
            if debug: print("Will run gen-level variables as well")
        ...
        if self.isMC:
          ### All the weights ###
          self.btagweights = doBTagWeights(self.events, self.jets, self.era, "L", do_syst=self.do_syst) # Does not change selection 
          self.puweights   = self.doPUWeights(self.events)              # Does not change selection
          self.l1preweights= self.doPrefireWeights(self.events)
          self.triggerSFs  = doTriggerSFs(self.electrons, self.muons, self.era, self.do_syst)
        ...
        self.tracksVar = {"": self.tracks} # If not activated, always central track collection
        ...
        if self.do_syst:
          ...
          if self.isSignal:
            ...
            self.lepsVar     = doLeptonScaleVariations(self.events, self.leptons, self.era)

          if self.doTracks: 
            if debug: print("Start tracks dropping")
        ...
        for var in self.varsToDo:
            ...
            if len(self.varsToDo) > 1: 
                ...
                if var in self.jetsVar:
                  if debug: print("Replacing jets for var %s"%var)
                  self.jets   = self.jetsVar[var]
                if var in self.tracksVar:
                  if debug: print("Replacing tracks for var %s"%var)
                  self.tracks = self.tracksVar[var]
                if var in self.lepsVar:
                  if debug: print("Replacing leptons for var %s"%var)
    ...
    def doAllPlots(self, channel, debug=True):
        ...
        out["nBLoose"]        = ak.sum((self.jets.btag >= btagcuts("Loose", self.era)), axis=1)[:]
        out["nBMedium"]       = ak.sum((self.jets.btag >= btagcuts("Medium", self.era)), axis=1)[:]
        out["nBTight"]        = ak.sum((self.jets.btag >= btagcuts("Tight", self.era)), axis=1)[:]

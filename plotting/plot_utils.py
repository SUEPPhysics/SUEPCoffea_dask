...
def plot1d(h, ax, label, rebin=-1, color="default", lw=1):
    ...
    x = x[:-1]
    if rebin != -1:
        x, y, e = combine_bins(x, y, e, n=rebin)
...
def plot_ratio(
    ...
    if rebin != -1:
        x1, y1, y1_errs = combine_bins(x1, y1, y1_errs, n=rebin)
    ...
    if rebin != -1:
        x2, y2, y2_errs = combine_bins(x2, y2, y2_errs, n=rebin)
...
def plot_ratio_regions(
    ...
    for i, r in enumerate(regions):
        ...
        if rebin != -1:
            x1, y1, y1_errs = combine_bins(x1, y1, y1_errs, n=rebin)
        if i == 0:
            ax1.step(x1, y1, color="maroon", label=sample1, where="mid")
        else:
            ax1.step(x1, y1, color="maroon", where="mid")
        ax1.errorbar(
            x1, y1, yerr=y1_errs, color="maroon".upper(), fmt="", drawstyle="steps-mid"
        )
        ...
        if rebin != -1:
            x2, y2, y2_errs = combine_bins(x2, y2, y2_errs, n=rebin)
        if i == 0:
            ax1.step(x2, y2, color="blue", label=sample2, where="mid")
        else:
            ax1.step(x2, y2, color="blue", where="mid")
        ax1.errorbar(
            x2, y2, yerr=y2_errs, color="blue".upper(), fmt="", drawstyle="steps-mid"
        )

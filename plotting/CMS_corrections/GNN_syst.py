...
def apply_GNN_syst(plots, fGNNsyst, models, bins, era, out_label='GNN'):
    ...
    for model in models:
        ...
        if yearSyst is None:
            logging.warning("--- {} was not found in file {}; systematic has not been applied".format(era, fGNNsyst))
            continue
        ...
        for plot in plots.keys():
            # apply only to GNN
            if not plot.endswith(out_label): continue

            if model in plot and '2D' not in plot:
                GNN_syst_plots[plot+"_GNNsyst_down"] = plot_utils.apply_binwise_scaling(plots[plot].copy(), bins, [1-s for s in scales])
                GNN_syst_plots[plot+"_GNNsyst_up"] = plot_utils.apply_binwise_scaling(plots[plot].copy(), bins, [1+s for s in scales])
            if model in plot and '2D' in plot:
                ...
                GNN_syst_plots[plot+"_GNNsyst_down"] = plot_utils.apply_binwise_scaling(plots[plot].copy(), bins, [1-s for s in scales],dim=dim)
                GNN_syst_plots[plot+"_GNNsyst_up"] = plot_utils.apply_binwise_scaling(plots[plot].copy(), bins, [1+s for s in scales],dim=dim)

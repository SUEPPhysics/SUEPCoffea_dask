...
for ifile in tqdm(files):
    ...
    # apply selections
    df_ML = df.copy()
    for sel in selections_ML: 
        df_ML = apply_selection(df_ML, sel[0], sel[1], sel[2])
        
    # keep event wide and ML variables, cut out events that don't pass IRM
    df_ML = df_ML[[c for c in df_ML.keys() if 'CL' not in c and 'IRM' not in c]]
    df_ML = df_ML[~df_ML['resnet_SUEP_pred_ML'].isnull()]
    ...
    # apply selections
    df_IRM = df.copy()
    for sel in selections_IRM: 
        df_IRM = apply_selection(df_IRM, sel[0], sel[1], sel[2])
        
    # keep event wide and IRM variables, cut out events that don't pass IRM
    df_IRM = df_IRM[[c for c in df_IRM.keys() if 'ML' not in c and 'CL' not in c]]
    df_IRM = df_IRM[~df_IRM['SUEP_pt_IRM'].isnull()]
    ...
    output["2D_SUEP_S1_ntracks_"+label].fill(df_IRM["SUEP_S1_"+label], df_IRM["ntracks"], weight=df_IRM['event_weight'])
    ...
    output["2D_ntracks_SUEP_pt_avg_"+label].fill(df_IRM["ntracks"], df_IRM["SUEP_pt_avg_"+label], weight=df_IRM['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_avg_"+label].fill(df_IRM["SUEP_nconst_"+label], df_IRM["SUEP_pt_avg_"+label], weight=df_IRM['event_weight'])
    output["2D_SUEP_S1_SUEP_pt_avg_b_"+label].fill(df_IRM["SUEP_S1_"+label], df_IRM["SUEP_pt_avg_b_"+label], weight=df_IRM['event_weight'])
    output["2D_ntracks_SUEP_pt_avg_b_"+label].fill(df_IRM["ntracks"], df_IRM["SUEP_pt_avg_b_"+label], weight=df_IRM['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_avg_b_"+label].fill(df_IRM["SUEP_nconst_"+label], df_IRM["SUEP_pt_avg_b_"+label], weight=df_IRM['event_weight'])
    output["2D_SUEP_S1_SUEP_pt_mean_scaled_"+label].fill(df_IRM["SUEP_S1_"+label], df_IRM["SUEP_pt_mean_scaled_"+label], weight=df_IRM['event_weight'])
    output["2D_ntracks_SUEP_pt_mean_scaled_"+label].fill(df_IRM["ntracks"], df_IRM["SUEP_pt_mean_scaled_"+label], weight=df_IRM['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_mean_scaled_"+label].fill(df_IRM["SUEP_nconst_"+label], df_IRM["SUEP_pt_mean_scaled_"+label], weight=df_IRM['event_weight'])
    ...
    # apply selections
    df_CL = df.copy()
    for sel in selections_CL: 
        df_CL = apply_selection(df_CL, sel[0], sel[1], sel[2])
        
    # keep event wide and IRM variables, cut out events that don't pass IRM
    df_CL = df_CL[[c for c in df_CL.keys() if 'ML' not in c and 'IRM' not in c]]
    df_CL = df_CL[~df_CL['SUEP_pt_CL'].isnull()]
    
    # blind
    if options.blind and not options.isMC:
         df_CL = df_CL.loc[((df_CL[var1_CL] < var1_CL_val) & (df_CL[var2_CL] < var2_CL_val)) | ((df_CL[var1_CL] >= var1_CL_val) & (df_CL[var2_CL] < var2_CL_val)) | ((df_CL[var1_CL] < var1_CL_val) & (df_CL[var2_CL] >= var2_CL_val))]

    # divide the dfs by region
    df_A = df_CL.loc[(df_CL[var1_CL] < var1_CL_val) & (df_CL[var2_CL] < var2_CL_val)]
    df_B = df_CL.loc[(df_CL[var1_CL] >= var1_CL_val) & (df_CL[var2_CL] < var2_CL_val)]
    df_C = df_CL.loc[(df_CL[var1_CL] < var1_CL_val) & (df_CL[var2_CL] >= var2_CL_val)]
    df_D_obs = df_CL.loc[(df_CL[var1_CL] >= var1_CL_val) & (df_CL[var2_CL] >= var2_CL_val)]
    ...
    # fill the ABCD histograms for both variables
    output["A_"+label].fill(df_A[var1_CL], weight=df_A['event_weight'])
    output["B_"+label].fill(df_B[var1_CL], weight=df_B['event_weight'])
    output["D_exp_"+label].fill(df_B[var1_CL], weight=df_B['event_weight'])
    output["C_"+label].fill(df_C[var1_CL], weight=df_C['event_weight'])
    output["D_obs_"+label].fill(df_D_obs[var1_CL], weight=df_D_obs['event_weight'])
    output["A_var2_"+label].fill(df_A[var2_CL], weight=df_A['event_weight'])
    output["B_var2_"+label].fill(df_B[var2_CL], weight=df_B['event_weight'])
    output["D_exp_var2_"+label].fill(df_C[var2_CL], weight=df_C['event_weight'])
    output["C_var2_"+label].fill(df_C[var2_CL], weight=df_C['event_weight'])
    output["D_obs_var2_"+label].fill(df_D_obs[var2_CL], weight=df_D_obs['event_weight'])
    output["ABCDvars_2D_"+label].fill(df_CL[var1_CL], df_CL[var2_CL], weight=df_CL['event_weight'])

    # fill the distributions as they are saved in the dataframes
    plot_labels = [key for key in df_CL.keys() if key in list(output.keys())]     # all the _IRM things
    for plot in plot_labels: output[plot].fill(df_CL[plot], weight=df_CL['event_weight'])  
    ...
    # fill some new distributions  
    output["2D_SUEP_S1_ntracks_"+label].fill(df_CL["SUEP_S1_"+label], df_CL["ntracks"], weight=df_CL['event_weight'])
    output["2D_SUEP_S1_SUEP_nconst_"+label].fill(df_CL["SUEP_S1_"+label], df_CL["SUEP_nconst_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_S1_SUEP_pt_avg_"+label].fill(df_CL["SUEP_S1_"+label], df_CL["SUEP_pt_avg_"+label], weight=df_CL['event_weight'])
    output["2D_ntracks_SUEP_pt_avg_"+label].fill(df_CL["ntracks"], df_CL["SUEP_pt_avg_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_avg_"+label].fill(df_CL["SUEP_nconst_"+label], df_CL["SUEP_pt_avg_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_S1_SUEP_pt_avg_b_"+label].fill(df_CL["SUEP_S1_"+label], df_CL["SUEP_pt_avg_b_"+label], weight=df_CL['event_weight'])
    output["2D_ntracks_SUEP_pt_avg_b_"+label].fill(df_CL["ntracks"], df_CL["SUEP_pt_avg_b_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_avg_b_"+label].fill(df_CL["SUEP_nconst_"+label], df_CL["SUEP_pt_avg_b_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_S1_SUEP_pt_mean_scaled_"+label].fill(df_CL["SUEP_S1_"+label], df_CL["SUEP_pt_mean_scaled_"+label], weight=df_CL['event_weight'])
    output["2D_ntracks_SUEP_pt_mean_scaled_"+label].fill(df_CL["ntracks"], df_CL["SUEP_pt_mean_scaled_"+label], weight=df_CL['event_weight'])
    output["2D_SUEP_nconst_SUEP_pt_mean_scaled_"+label].fill(df_CL["SUEP_nconst_"+label], df_CL["SUEP_pt_mean_scaled_"+label], weight=df_CL['event_weight'])

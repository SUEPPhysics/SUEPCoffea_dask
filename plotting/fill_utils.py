...
def prepareDataFrame(df, abcd, label_out, blind=True, isMC=False):
    ...
    if blind and not isMC:    
        ...
        if 'SR2' in abcd.keys(): df = blind_DataFrame(df, abcd['SR2'])
...
def auto_fill(df, output, abcd, label_out, isMC=False, do_abcd=False):
    ...
    event_plot_labels = [key for key in df.keys() if key+"_"+label_out in list(output.keys())]  
    for plot in event_plot_labels: output[plot+"_"+label_out].fill(df[plot], weight=df['event_weight']) 
    
    # 1b. Plot method variables
    method_plot_labels = [key for key in df.keys() if key.replace(input_method, label_out) in list(output.keys()) and key.endswith(input_method)]
        ...
        for i in range(len(xvar_regions)-1):
            ...
            for j in range(len(yvar_regions)-1):
                ...
                for plot in event_plot_labels: 
                    if r+plot+"_"+label_out not in list(output.keys()): continue
...
def apply_normalization(plots, xsection, total_gensumweight):
    if total_gensumweight > 0.0:
        for plot in list(plots.keys()): plots[plot] = plots[plot]*xsection/total_gensumweight
    else:

...
def prepare_DataFrame(
    ...
    if "selections" in config.keys():
        for sel in config["selections"]:
            if type(sel) is str: # converts "attribute operator value" to ["attribute", "operator", "value"] to pass to make_selection()
                sel = sel.split(" ")
            if sel[0] not in df.keys(): # error out if variable doesn't exist in the DataFrame
                raise Exception(
                    f"Trying to apply a cut on a variable {sel[0]} that does not exist in the DataFrame"
                )
            if type(sel[2]) is str and sel[2].isdigit():
                sel[2] = float(sel[2])  # convert to float if the value to cut on is a number
            ...
            cutflow_label = "cutflow_" + sel[0] + "_" + sel[1] + "_" + str(sel[2]) + "_" + label_out

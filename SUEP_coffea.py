        ...
        if not any(oneCOtrackCut):
            ...
            if self.isMC:
                ...
                if not any(oneCOISRtrackCut):
                    print(f"[{dataset}]: No events in Inverted CO Removal Method, oneCOISRtrackCut.")
                    for c in columns_CO_ISR:
                        out_vars[c] = np.nan
                else:
                    ...
                    out_vars.loc[indices_CO, "ISR_rho0_CO"] = math_utils.rho(
                        self, 0, ISR_cand_CO, ISR_tracks_CO, deltaR
                    )
                    out_vars.loc[indices_CO, "ISR_rho1_CO"] = math_utils.rho(
